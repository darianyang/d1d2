...
if [[ $(echo "if (${m1_rms} > ${m2_rms}) 1 else 0" | bc) -eq 1 ]]; then
    echo "${m1_rms} > ${m2_rms}"
    paste <(cat o_angle.dat | tail -n +2 | awk '{print $2}') <(cat M1_W184_chi12.dat | tail -n +2 | awk '{print $2}') <(cat M2_W184_chi12.dat | tail -n +2 | awk '{print $2}')> $WEST_PCOORD_RETURN
else
    echo "${m1_rms} < ${m2_rms}"
    paste <(cat o_angle.dat | tail -n +2 | awk '{print $3}') <(cat M1_W184_chi12.dat | tail -n +2 | awk '{print $2}') <(cat M2_W184_chi12.dat | tail -n +2 | awk '{print $2}')> $WEST_PCOORD_RETURN
fi
