...
###########################
f_d1d2_avg, f_d1d2_crs = grab_kin_from_files(f_d1d2_h5s, ax)
f_d2d1_avg, f_d2d1_crs = grab_kin_from_files(f_d2d1_h5s, ax)

errors = np.flip(np.rot90(np.vstack((f_d1d2_crs, [3.34, 3.34], f_d2d1_crs, [8.26, 8.26])), k=-1), axis=1)
...
ax.bar(["WE $k_{12}$", "NMR $k_{12}$", "WE $k_{21}$", "NMR $k_{21}$"], 
       [f_d1d2_avg, 60, f_d2d1_avg, 134.4], width=0.6,
       #color=["tab:blue", "tab:grey", "tab:orange", "tab:pink"],
       #color=["tab:pink", "tab:grey", "tab:pink", "tab:grey"],
       color=["tab:blue", "tab:grey", "tab:orange", "tab:grey"],
       yerr=errors, capsize=10, error_kw={"elinewidth":1.6, "capthick":1.6})
