...
#        self.workdir = os.environ('LOCAL')
    ...
    def process_iteration(self, n_iter, iter_group):
        ...
        iteration_data_array = numpy.zeros((nsegs, 75, 75))
        ...
        for iseg in range(nsegs):
            ...
            fin = open("wcrawl/cpp_inter_ca_dmat.temp", 'rt') 
            data = fin.read()
#            if n_iter == 1:
#                # bstates
...
#                data = data.replace('PARENT', parentpath)
            ...
            # sleep if needed
            while os.path.isfile(f"wcrawl/{aux_name}_{n_iter}_{iseg}.dat") is False:
                time.sleep(1)
            # sleep longer if needed
            #if os.path.isfile(f"wcrawl/{aux_name}_{n_iter}_{iseg}.dat") is False:
            #    time.sleep(5)
            ...
            # using np alt way of loading data and filling array
            dat = numpy.loadtxt(f"wcrawl/{aux_name}_{n_iter}_{iseg}.dat")
            iteration_data_array[iseg, :, :] = dat
