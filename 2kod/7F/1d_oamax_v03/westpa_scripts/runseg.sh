...
CMD="$CMD parm $WEST_SIM_ROOT/reference/2kod_7f_leap.pdb [nmrparm] \n"
CMD="$CMD reference $WEST_SIM_ROOT/reference/2kod_7f_leap.pdb parm [nmrparm] name [nmr] \n"
CMD="$CMD parm $WEST_SIM_ROOT/reference/1a43_7f_leap.pdb [xtalparm] \n"
CMD="$CMD reference $WEST_SIM_ROOT/reference/1a43_7f_leap.pdb parm [xtalparm] name [xtal] \n"
CMD="$CMD parm $WEST_SIM_ROOT/reference/hex_7f_leap.pdb [hexparm] \n"
CMD="$CMD reference $WEST_SIM_ROOT/reference/hex_7f_leap.pdb parm [hexparm] name [hex] \n"
CMD="$CMD parm $WEST_SIM_ROOT/reference/pent_7f_leap.pdb [pentparm] \n"
CMD="$CMD reference $WEST_SIM_ROOT/reference/pent_7f_leap.pdb parm [pentparm] name [pent] \n"
CMD="$CMD autoimage \n"
...
if [[ $(echo "if (${m1_rms} > ${m2_rms}) 1 else 0" | bc) -eq 1 ]]; then
    echo "${m1_rms} > ${m2_rms}"
    paste <(cat o_angle.dat | tail -n +2 | awk '{print $2}') > $WEST_PCOORD_RETURN
else
    echo "${m1_rms} <= ${m2_rms}"
    paste <(cat o_angle.dat | tail -n +2 | awk '{print $3}') > $WEST_PCOORD_RETURN
fi
